...
export const usersService = {
  ...
  async searchUsers({
    ...
    if (filters.username) {
      body.username = { filter_value: filters.username, comparator: 'contains' }
    }
    if (filters.email) {
      body.email = { filter_value: filters.email, comparator: 'contains' }
    }
    if (filters.firstName) {
      body.first_name = { filter_value: filters.firstName, comparator: 'contains' }
    }
    if (filters.lastName) {
      body.last_name = { filter_value: filters.lastName, comparator: 'contains' }
    }
    if (filters.isActive !== '') {
